...
function render() {
  ...
  const applyState = (state) => {
    ...
    foocusLastAppliedStatus = status;

    container.style.visibility = isSnoozed ? "hidden" : "visible";

    const focusInput = shadowRoot.getElementById("focus-input");
    if (focusInput) focusInput.value = focusText ?? "";

    const mins = Math.floor(remainingTime / 60);
    const secs = remainingTime % 60;
    shadowRoot.getElementById("timer-min").value = String(mins).padStart(3, "0");
    shadowRoot.getElementById("timer-sec").value = String(secs).padStart(2, "0");

    const progressEl = shadowRoot.querySelector(".progress");
    let pct =
      totalInitialTime > 0
    ...
    if (progressColor) progressEl.style.backgroundColor = progressColor;

    const playBtn = shadowRoot.querySelector(".play-btn");
    playBtn.innerHTML =
      status === "RUNNING" ? FOOCUS_PAUSE_ICON_SVG : FOOCUS_PLAY_ICON_SVG;

    const containerEl = shadowRoot.querySelector(".container");
    const notchEl = shadowRoot.querySelector(".notch");
    containerEl.classList.toggle("collapsed", isCollapsed);
    const notchSvg = notchEl.querySelector("svg");
    if (isCollapsed) {
      notchSvg.style.transform = "rotate(180deg) translateX(50%)";
      const menu = shadowRoot.querySelector(".dropdown-menu");
      if (menu) menu.classList.add("hidden");
    } else {
      notchSvg.style.transform = "";
    }

    containerEl.classList.toggle("dark-mode", isDarkMode);
    notchEl.classList.toggle("dark-mode", isDarkMode);
    const darkToggle = shadowRoot.querySelector("#darkmode-toggle");
    if (darkToggle) darkToggle.checked = isDarkMode;

    progressEl.classList.toggle("shim", animationEnabled);
    const animToggle = shadowRoot.querySelector("#animation-toggle");
    if (animToggle) animToggle.checked = animationEnabled;

    if (status === "RUNNING") {
      if (!foocusDisplaySyncInterval) {
        foocusDisplaySyncInterval = setInterval(() => {
          chrome.runtime.sendMessage({ action: "getState" }, (s) => {
            if (chrome.runtime.lastError) return;
            if (s && typeof s.status === "string" && foocusApplyState) {
              foocusApplyState(s);
            }
          });
        }, 1000);
      }
    } else if (foocusDisplaySyncInterval) {
      clearInterval(foocusDisplaySyncInterval);
      foocusDisplaySyncInterval = null;
    }
  };

  foocusApplyState = applyState;

  chrome.runtime.sendMessage({ action: "getState" }, (state) => {
    if (chrome.runtime.lastError) return;
    if (state && typeof state.status === "string") applyState(state);
  });
  ...
  const sendTimerUpdate = () => {
    const minutes =
      parseInt(shadowRoot.getElementById("timer-min").value, 10) || 0;
    const seconds =
      parseInt(shadowRoot.getElementById("timer-sec").value, 10) || 0;
  ...
  const timerMinEl = shadowRoot.getElementById("timer-min");
  const timerSecEl = shadowRoot.getElementById("timer-sec");
  timerMinEl.addEventListener("input", validateTimerInput);
  timerSecEl.addEventListener("input", validateTimerInput);
  timerMinEl.addEventListener("blur", sendTimerUpdate);
  timerSecEl.addEventListener("blur", sendTimerUpdate);
  const onTimerKeydown = (e) => {
    if (e.key === "Enter") {
      e.target.blur();
    }
  };
  timerMinEl.addEventListener("keydown", onTimerKeydown);
  timerSecEl.addEventListener("keydown", onTimerKeydown);

  shadowRoot.getElementById("focus-input").addEventListener("input", (e) => {
    chrome.runtime.sendMessage({
      action: "updateFocus",
      payload: { focusText: e.target.value },
    });
  });

  shadowRoot.querySelector(".play-btn").addEventListener("click", () => {
    if (foocusLastAppliedStatus === "RUNNING") {
      chrome.runtime.sendMessage({ action: "pause" });
    } else {
      chrome.runtime.sendMessage({ action: "play" });
    }
  });
  shadowRoot.querySelector(".reset-btn").addEventListener("click", () => {
  ...
  notch.addEventListener("click", () => {
    chrome.runtime.sendMessage({ action: "toggleCollapse" });
  });
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "render") {
    render();
  } else if (request.action === "stateUpdate" && foocusApplyState) {
    foocusApplyState(request.state);
